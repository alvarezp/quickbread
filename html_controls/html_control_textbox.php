...
class HtmlControlTextbox extends HtmlControl {
	...
	public function get_html_editable($basename) {
		...
		if (isset($this->value)) {
			...
			$r .= "<input id='${basename}[aft][value]' type='text' name='${basename}[aft][value]' class='textbox' value='$this->value' $required_html></input>\n";
		} else {
			$r .= "<input type='hidden' name='${basename}[bef][value]' value=''></input>\n";
			$r .= "<input id='${basename}[aft][value]' type='text' name='${basename}[aft][value]' class='textbox' value='' $required_html></input>\n";
		}
		$r .= "</span>";
